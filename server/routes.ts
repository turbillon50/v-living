...
export async function registerRoutes(
  ...
  // Clerk Webhook - receives events directly from Clerk (user.created, user.updated, user.deleted)
  app.post("/api/clerk/webhook", async (req, res) => {
    ...
    try {
      ...
      console.log(`[Clerk Webhook] Event: ${eventType}, User: ${data.id}`);
      ...
      if (eventType === "user.created" || eventType === "user.updated") {
        ...
        const clerkId = data.id;
        const users = await storage.getUsers();
        const user = users.find(u => u.password === clerkId);
        if (user) {
          await storage.updateUser(user.id, { status: "inactive" } as any);
          console.log(`[Clerk Webhook] Marked user inactive: ${user.email}`);
        } else {
          console.log(`[Clerk Webhook] Deleted user not found in DB: ${clerkId}`);
        }
      }
